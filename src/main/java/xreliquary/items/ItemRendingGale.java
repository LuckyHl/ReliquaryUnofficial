 ...
 */
@ContentInit
public class ItemRendingGale extends ItemToggleable implements xreliquary.api.INbtSyncTolerant {
    ...
    /**
     */
    private static final int ABSORB_INTERVAL = 1;
    ...
    /**
     */
    public static boolean isHoldToUse() { return Reliquary.CONFIG.getBool(Names.rending_gale, "hold_to_use"); }
    ...
    public void onUpdate(ItemStack ist, World world, Entity e, int i, boolean b) {
        ...
        if (!this.isEnabled(ist))
            return;
        if (world.getTotalWorldTime() % ABSORB_INTERVAL != 0)
            return;
        int current = NBTHelper.getInteger("feathers", ist);
        int worth = getFeathersWorth();
        int absorbed = 0;
        while (absorbed < ABSORB_INTERVAL && current + worth <= getChargeLimit()) {
        ...
        if (absorbed > 0) {
            NBTHelper.setInteger("feathers", ist, current);
        }
    }
    ...
    public void onUsingTick(ItemStack ist, EntityPlayer player, int count) {
        if (isHoldToUse()) {
            int feathers = NBTHelper.getInteger("feathers", ist);
            if (feathers < getChargeCost()) {
                player.stopUsingItem();
                return;
            }
            NBTHelper.setInteger("feathers", ist, feathers - getChargeCost());
        } else {
