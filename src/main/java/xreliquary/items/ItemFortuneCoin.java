...
@ContentInit
public class ItemFortuneCoin extends ItemBauble implements xreliquary.api.INbtSyncTolerant {
    ...
    public void onUpdate(ItemStack ist, World world, Entity entity, int i, boolean f) {
        if (world.isRemote)
            return;
        if (!disabledAudio()) {
            int soundStart = NBTHelper.getInteger("soundTimerStart", ist);
            if (soundStart > 0) {
                int elapsed = (int) world.getTotalWorldTime() - soundStart;
                if (elapsed >= 0 && elapsed < 6 && elapsed % 2 == 0) {
    ...
    public static boolean isHoldToUse() {
        return Reliquary.CONFIG.getBool(Names.fortune_coin, "hold_to_use");
    }
    ...
    public ItemStack onItemRightClick(ItemStack ist, World world, EntityPlayer player) {
        if (player.isSneaking()) {
            if (!disabledAudio()) {
                NBTHelper.setInteger("soundTimerStart", ist, (int) world.getTotalWorldTime());
            }
