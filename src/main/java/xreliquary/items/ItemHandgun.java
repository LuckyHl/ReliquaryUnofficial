...
@ContentInit
public class ItemHandgun extends ItemBase implements xreliquary.api.INbtSyncTolerant {
    ...
    public int getCooldown(ItemStack ist, World world) {
        int duration = NBTHelper.getShort("cooldownDuration", ist);
        if (duration <= 0 || world == null)
            return 0;
        int start = NBTHelper.getInteger("cooldownStart", ist);
        int elapsed = (int) world.getTotalWorldTime() - start;
        return Math.max(0, duration - elapsed);
    }

    public void setCooldown(ItemStack ist, World world, int ticks) {
        NBTHelper.setInteger("cooldownStart", ist, (int) world.getTotalWorldTime());
        NBTHelper.setShort("cooldownDuration", ist, (short) ticks);
    }
    ...
    public ItemStack onItemRightClick(ItemStack ist, World worldObj, EntityPlayer player) {
        if (getCooldown(ist, worldObj) <= 0) {
            if (!(getBulletCount(ist) > 0) && !(getBulletType(ist) > 0)) {
                player.setItemInUse(ist, this.getMaxItemUseDuration(ist));
            } else {
                if (!worldObj.isRemote) {
                    setCooldown(ist, worldObj, Reference.PLAYER_HANDGUN_SKILL_MAXIMUM + Reference.HANDGUN_COOLDOWN_SKILL_OFFSET - Math.min(player.experienceLevel, Reference.PLAYER_HANDGUN_SKILL_MAXIMUM));
    ...
    public void onUsingTick(ItemStack ist, EntityPlayer player, int unadjustedCount) {
        ...
        if (!hasFilledMagazine(player)) {
            ...
            setCooldown(ist, player.worldObj, 12);
